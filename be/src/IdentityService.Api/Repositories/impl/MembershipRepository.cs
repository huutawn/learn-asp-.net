...
public sealed class MembershipRepository(ApplicationDbContext dbContext) : IMembershipRepository
{
    ...
    public Task<PrincipalMembership?> GetAsync(Guid userId, Guid principalId, CancellationToken ct) => dbContext.PrincipalMemberships.FindAsync([userId, principalId], ct).AsTask();
    ...
    public async Task<bool> HasPermissionAsync(Guid userId, string permission, Guid resourcePrincipalId, CancellationToken ct) => (await GetPermissionsAsync(userId, resourcePrincipalId, ct)).Contains(permission);
    public async Task<IReadOnlyList<string>> GetPermissionNamesByIdsAsync(IEnumerable<Guid> ids, CancellationToken ct) => await dbContext.Permissions.Where(x => ids.Contains(x.Id)).Select(x => x.Name).ToListAsync(ct);
    public async Task<IReadOnlyList<string>> GetRolePermissionNamesByIdsAsync(IEnumerable<Guid> ids, CancellationToken ct) => await dbContext.RolePermissions.Where(x => ids.Contains(x.RoleId)).Select(x => x.Permission.Name).Distinct().ToListAsync(ct);
    public async Task<(IReadOnlyList<Guid> RoleIds, IReadOnlyList<Guid> PermissionIds)> GetAccessAsync(Guid subjectPrincipalId, Guid resourcePrincipalId, CancellationToken ct) => (await dbContext.RoleAssignments.Where(x => x.SubjectPrincipalId == subjectPrincipalId && x.ResourcePrincipalId == resourcePrincipalId).Select(x => x.RoleId).ToListAsync(ct), await dbContext.PermissionGrants.Where(x => x.SubjectPrincipalId == subjectPrincipalId && x.ResourcePrincipalId == resourcePrincipalId).Select(x => x.PermissionId).ToListAsync(ct));
    ...
    public Task<bool> HasAnotherOwnerAsync(Guid principalId, Guid excludedUserId, CancellationToken ct) => dbContext.PrincipalMemberships.AnyAsync(x => x.PrincipalId == principalId && x.UserId != excludedUserId && x.LeftAtUtc == null && x.IsOwner, ct);
